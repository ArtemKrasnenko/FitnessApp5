...
struct AddViewB: View {
    ...
    var body: some View {
        let TaskEatBreakfastFull = [
            ["Полезные банановые панкейки", "BananaCake", 230, CalendarModel.currentDay],
            ["Полезная ватрушка с творогом", "CheeseCake", 154, CalendarModel.currentDay],
            ["Омлет с овощами", "Omelet", 108, CalendarModel.currentDay],
            ["Молочная рисовая каша", "RicePorridge", 97, CalendarModel.currentDay],
            ["Овсяноблин с разными начинками", "Oatmeal", 161, CalendarModel.currentDay],
            ["Тост с яйцом-пашот и авакодо", "ToastWithEgg", 190, CalendarModel.currentDay]]
        ...
        VStack() {
            ...
            TextField("Колличество каллорий", text: $calloriesProduct)
                ...
                Button {
                    self.showingB.toggle()
                    
                    if nameProduct != "" && calloriesProduct != "" {
                        realManager.addEat(EatTitle: nameProduct, EatImage: "Rectangle5919", EatCallories: Int(calloriesProduct) ?? 0, EatDate: CalendarModel.currentDay)
                    }
            ...
            Button {
                ...
                let a = TaskEatBreakfastFull.randomElement()!
